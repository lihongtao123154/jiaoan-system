...
c.on('ready',function(){
  c.exec('kubectl get pod -n '+NS+' -o jsonpath="{.items[0].metadata.name}"',function(e,s){
    var pod='';s.on('data',function(d){pod+=d});s.on('close',function(){
      pod=pod.trim();
      c.exec('kubectl exec -n '+NS+' '+pod+' -- kill 19 2>/dev/null; kubectl exec -n '+NS+' '+pod+' -- kill 22 2>/dev/null',function(e,s){
        var o='';s.on('data',function(d){o+=d});s.stderr.on('data',function(d){o+=d});
        s.on('close',function(){console.log('Killed node. Waiting for restart...');setTimeout(function(){
          c.exec('kubectl exec -n '+NS+' '+pod+' -- ps aux | grep node',function(e,s2){
            var o2='';s2.on('data',function(d){o2+=d});s2.stderr.on('data',function(d){o2+=d});
            s2.on('close',function(){console.log('After restart:',o2.trim());c.end()});
          });
        },8000);c.end()});
      });
    });
